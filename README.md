...
- `includeRequiredPolyfill` allows explicit inclusion of polyfills deemed necessary for the library’s functionality. By specifying the required polyfills, they will be automatically included to ensure proper execution. These polyfills are applied at runtime and are not bundled directly within the library. Therefore, `core-js-pure` must be declared in `dependencies` with a floor of at least `3.39.0`, the version pite generates polyfill paths for. pite checks this when the config is created and fails the build otherwise.
